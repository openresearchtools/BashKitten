...
def ensure_private_directory(path: Path) -> Path:
    return _private_directory(path.expanduser().absolute())


def document_directory() -> Path:
    configured = os.environ.get("BASHKITTEN_DATA_DIR")
    root = Path(configured).expanduser() if configured else Path.home() / ".local/share/bashkitten-pi"
    if not root.is_absolute():
        raise RuntimeError("BASHKITTEN_DATA_DIR must be absolute")
    return ensure_private_directory(root / "search/documents")
...
def title_for_document(markdown: str, source_url: str, preferred: str | None = None) -> str:
    ...
    headings = re.findall(r"(?m)^(#{1,6})\s+(.+?)\s*$", markdown[:32_000])
    ordered = [value for marks, value in headings if len(marks) == 1]
    ordered.extend(value for marks, value in headings if len(marks) > 1)
    heading = next(
        (
            value.strip(" #")
            for value in ordered
            if not re.fullmatch(r"(?i)page\s+\d+", value.strip(" #"))
        ),
        None,
    )
    path_name = unquote(parsed.path.rstrip("/").rsplit("/", 1)[-1])
    path_name = re.sub(r"\.(?:html?|md|pdf|txt)$", "", path_name, flags=re.I)
    title = preferred or heading or path_name or parsed.hostname or "page"
...
def write_markdown(title: str, markdown: str, output_directory: Path | None = None) -> Path:
    ...
    if prefix_budget < 1:
        raise RuntimeError("Search document path has no safe filename space")
    prefix = _safe_title(title)[:prefix_budget].rstrip("-.") or "page"
    filename = f"{prefix}{suffix}"
    directory_descriptor = os.open(directory, os.O_RDONLY | os.O_DIRECTORY | os.O_CLOEXEC | getattr(os, "O_NOFOLLOW", 0))
    descriptor = -1
    ...
    if len(os.fsencode(path)) >= path_max:
        raise RuntimeError("Search document path exceeded PATH_MAX")
    return path

...
function settings(value) {
  ...
  if (result.model && (!path.isAbsolute(result.model) || result.model.length > 8192)) throw Error('Choose an absolute GGUF model path');
...
async function responseJson(url, apiKey, timeout = 5000) {
  const response = await fetch(url, { headers: apiKey ? { Authorization: `Bearer ${apiKey}` } : {}, redirect: 'error', signal: AbortSignal.timeout(timeout) });
...
export async function probeLlamaEndpoint({ url, apiKey, model }) {
  ...
  if (!Array.isArray(result.data) || !result.data.length) throw Error('llama.cpp has no loaded model');
  ...
  return { ready: true, model: selected.id, models: result.data.map(value => ({ id: value.id, meta: value.meta })) };
}
...
export class ManagedLlama {
  constructor(directory = path.join(dataDir, 'llama')) {
    ...
    this.keyFile = path.join(directory, 'api-key'); this.listeners = new Set(); this.desired = false; this.serial = Promise.resolve();
    this.generation = 0; this.failures = []; this.current = { state: 'stopped', desired: false }; this.config = { ...defaults };
  }
  ...
  async connection() {
    ...
    return { url: this.current.url, baseUrl: this.current.url + '/v1', model: this.config.alias, apiKey: await this.key() };
  }
  ...
  async start(value) {
    ...
    return this.exclusive(async () => {
      ...
      if (this.desired && this.launchSignature === signature && ['starting', 'loading', 'ready'].includes(this.current.state)) return this.status();
  ...
  async alive(info) {
    ...
    try { const argv = (await fs.readFile(`/proc/${info.pid}/cmdline`, 'utf8')).split('\0'); return argv.includes(this.keyFile) && argv.includes('--api-key-file') && argv.includes(info.model); } catch { return false; }
  }
  async stopChild() {
    ...
    if (await this.alive(info)) {
      ...
      for (let i = 0; i < 50 && await this.alive(info); i++) await sleep(100);
    ...
    this.current = { state: 'stopped', desired: false }; this.emit({});
  }
  async log(bytes, key) {
  ...
  async launch() {
    ...
    if (!config.model) throw Error('Choose a GGUF model before starting llama.cpp');
    const model = await fs.realpath(config.model);
    if (!(await fs.stat(model)).isFile()) throw Error('Choose a GGUF model file');
    ...
    const args = ['--host', '127.0.0.1', '--port', String(port), '--model', model, '--alias', config.alias, '--ctx-size', String(config.contextSize), '--api-key-file', this.keyFile, '--device', device?.id || 'none', '--gpu-layers', String(backend === 'cpu' ? 0 : config.gpuLayers), '--jinja'];
    ...
    if (!started) return;
    this.processInfo = { pid: child.pid, started, model, port };
    ...
    this.emit({ state: 'loading', pid: child.pid, backend, device: device?.description || 'CPU', model: config.alias, url: `http://127.0.0.1:${port}` });
    void this.watchReady(generation, key);
  }
  async watchReady(generation, key) {
    const deadline = Date.now() + 15 * 60 * 1000;
    let failure = '';
    while (this.desired && generation === this.generation && Date.now() < deadline) {
      try {
        await probeLlamaEndpoint({ url: this.current.url, apiKey: key, model: this.config.alias });
        if (this.desired && generation === this.generation && await this.alive(this.processInfo)) { this.readyAt = Date.now(); this.emit({ state: 'ready', error: null }); }
        return;
      } catch (error) { failure = error.message; }
      await sleep(750);
    }
    if (this.desired && generation === this.generation) {
      // A loading timeout is a failed configuration, not a reason to loop forever.
      await this.stop(); this.emit({ state: 'failed', error: `Model did not become ready: ${failure}` });
    }
  }
  crashed(error, generation) {
    ...
    if (!exhausted) this.retry = setTimeout(() => {
      if (this.desired && generation === this.generation) void this.exclusive(() => this.launch()).catch(value => this.emit({ state: 'failed', error: value.message, retryAt: null }));
    }, delay);
  }
  async waitReady({ timeout = 15 * 60 * 1000 } = {}) {
    const deadline = Date.now() + timeout;
    while (Date.now() < deadline) {
      if (this.current.state === 'ready' && this.desired) return this.connection();
      if (!this.desired || (this.current.state === 'failed' && !this.current.retryAt)) throw Error(this.current.error || 'Managed llama.cpp is stopped');
      await sleep(250);
    }

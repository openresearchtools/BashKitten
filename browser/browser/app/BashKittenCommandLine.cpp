...
constexpr const char* kCommands[] = {
    "help", "h", "version", "tools", "skill", "status", "agent_json",
};
...
std::string CommandName(int argc, char* argv[]) {
  for (int i = 1; i < argc; ++i) {
    ...
    if (argument == "--agent-json") return "agent_json";
    if (argument == "--json" || argument == "--no-start") {
      continue;
    }
    if (argument == "--cwd" || argument == "--input") {
      ++i;
      continue;
    }
...
bool SocketTarget(std::string& path, bool& explicitPath, std::string& error) {
  path = Env("BASHKITTEN_CONTROL_SOCKET");
  explicitPath = !path.empty();
  if (explicitPath) {
    if (!IsAbsolutePath(path) || path == "/") {
      error = "BASHKITTEN_CONTROL_SOCKET must be a normalized absolute path";
      return false;
    }
    return true;
  }
  ...
  if (IsAbsolutePath(runtime)) {
    path = runtime + "/bashkitten/control";
    return true;
  }
  ...
  path = state + "/bashkitten/run/control";
  return true;
}

bool IsSocketName(const std::string& name) {
  if (name.size() != 25 || name.substr(0, 8) != "control-" ||
      name.substr(20) != ".sock") {
    return false;
  }
...
bool PrivateDirectory(const std::string& path, bool allowMissing,
  ...
  if (!S_ISDIR(info.st_mode) || info.st_uid != geteuid() ||
      (info.st_mode & 0077)) {
    error =
        "BashKitten control directory must be owned by this user and "
        "private: " +
        path;
    return false;
  }
...
bool WaitForSocket(int fd, short events, Clock::time_point deadline,
  ...
  while (true) {
    auto remaining = std::chrono::duration_cast<std::chrono::milliseconds>(
                         ...
                         .count();
    if (remaining <= 0) {
      error = "timed out waiting for BashKitten";
      return false;
    }
    ...
    if (result < 0 && errno != EINTR) {
      error =
          "could not wait for BashKitten: " + std::string(strerror(errno));
      return false;
    }
  }
}
...
int ConnectSocket(const std::string& path, std::string& error) {
  ...
  if (path.size() >= sizeof(address.sun_path)) {
    error = "BashKitten control socket path is too long: " + path;
    return -1;
  }
  struct stat info;
  if (lstat(path.c_str(), &info) < 0) {
  ...
  if (!S_ISSOCK(info.st_mode) || info.st_uid != geteuid() ||
      (info.st_mode & 0077)) {
    error =
        "BashKitten control socket must be owned by this user and private: " +
        path;
    return -1;
  }
  ...
  if (result < 0) {
    if (errno != ENOENT && errno != ECONNREFUSED) {
      error =
          "could not connect to BashKitten: " + std::string(strerror(errno));
    }
    return -1;
  }
...
int ConnectExisting(const std::string& target, bool explicitPath,
  ...
  if (count > 1) {
    error =
        "multiple BashKitten control endpoints are running; set "
        "BASHKITTEN_CONTROL_SOCKET to one of: " +
        connected;
    return -1;
  }
...
bool LaunchBrowser(std::string& error) {
  ...
  if (child == 0) {
    ...
    if (browser == 0) {
      ...
      if (nullFd >= 0) {
        ...
        execl("/proc/self/exe", "bashkitten", nullptr);
      }
    }
    if (browser <= 0) {
      int status = errno;
  ...
  if (child < 0) {
    error = "could not launch BashKitten: " + std::string(strerror(errno));
    return false;
  }
  output.Reset();
  int status = 0;
  ...
  if (count != 0 || !WIFEXITED(status) || WEXITSTATUS(status) != 0) {
    error = "could not launch BashKitten: " +
...
bool ReadStdin(std::string& source, std::string& error) {
  ...
  while (true) {
    ...
    if (source.size() > kMaxRequestBytes) {
      error = "stdin exceeds the BashKitten request limit";
      return false;
    }
  }
}
...
bool Exchange(int fd, const std::string& request, Json::Value& response,
  ...
  for (size_t sent = 0; sent < request.size();) {
    ...
    if (count <= 0) {
      error = "could not send the BashKitten command";
      return false;
    }
    sent += count;
  }
  ...
  while (true) {
    ...
    if (count <= 0) {
      error = "BashKitten closed the connection without a complete response";
      return false;
    }
    source.append(buffer, count);
    if (source.size() > kMaxResponseBytes) {
      error = "BashKitten response exceeded its limit";
      return false;
    }
    if (source.find('\n') != std::string::npos) {
      break;
  ...
  if (!reader->parse(source.data(), source.data() + source.size(), &response,
    ...
    error = "invalid response from BashKitten";
    return false;
  }
  return true;
}
...
bool Call(int argc, char* argv[], const std::string& command,
  ...
  if (argc > 4097) {
    error = "too many BashKitten command arguments";
    return false;
  }
  std::string target;
  bool explicitPath;
  ...
  if (source.size() > kMaxRequestBytes) {
    error = "BashKitten command request is too large";
    return false;
  }
  ...
  if (connection.Get() < 0) {
    if (!start) {
      error = "BashKitten is not running";
      return false;
    }
    ...
    if (connection.Get() < 0) {
      error = "BashKitten did not expose native control at " + target +
              ": timed out";
      return false;
    }
  }
...
bool HandleBashKittenCommandLine(int argc, char* argv[], int& exitCode) {
  ...
  if (!Call(argc, argv, command, response, error)) {
    if (HasArgument(argc, argv, "--json") || HasArgument(argc, argv, "--agent-json")) {
      Json::Value value(Json::objectValue);
      value["error"]["code"] = "browser_unavailable";
      value["error"]["message"] = error;
      fputs(Encode(value).c_str(), stdout);
    } else {
      fprintf(stderr, "bashkitten: %s\n", error.c_str());
    }
    exitCode = 1;
    return true;
  }

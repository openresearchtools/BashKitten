 ...
 */
export class BashKittenBlockerParent extends JSWindowActorParent {
  /**
  ...
  receiveMessage(message) {
    switch (message.name) {
      case "BashKittenBlocker:GetCosmeticResources":
        return this._getCosmeticResources(message.data);
      case "BashKittenBlocker:GetHiddenClassIdSelectors":
  ...
  async _getCosmeticResources({ url } = {}) {
    ...
    await BashKittenBlockerService.whenEngineReady();

    const resources = BashKittenBlockerService.getCosmeticResources(
      url,
  ...
  _getHiddenClassIdSelectors({ classes, ids, exceptions } = {}) {
    if (!BashKittenBlockerService.isAndroidTabBlockingEnabled(this.browsingContext?.top?.browserId)) return [];
    return BashKittenBlockerService.getHiddenClassIdSelectors(
      classes || [],
      ids || [],
      exceptions || []
    );

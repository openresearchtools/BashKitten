 ...
 */
export class BashKittenBlockedPageChild extends JSWindowActorChild {
  handleEvent(event) {
    ...
    this.sendQuery("BashKittenBlockedPage:AllowAndNavigate", {
      url: blockedUrl,
    })
      .then(ok => {
        if (!ok) {
      ...
      .catch(err => {
        console.error(
          "[BashKittenBlockedPageChild] AllowAndNavigate failed:",
          err
        );
      });
  }

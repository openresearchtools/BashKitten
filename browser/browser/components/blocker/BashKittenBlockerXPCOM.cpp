...
#include "BashKittenBlockerXPCOM.h"
...
NS_IMPL_ISUPPORTS(BashKittenBlockerContentPolicy, nsIContentPolicy)
NS_IMPL_ISUPPORTS(BashKittenBlockerXPCOM, nsIBashKittenBlockerEngine)
...
BashKittenBlockerContentPolicy::BashKittenBlockerContentPolicy() = default;

BashKittenBlockerContentPolicy::~BashKittenBlockerContentPolicy() = default;

nsIBashKittenBlockerContentPolicyBridge*
BashKittenBlockerContentPolicy::GetBridge() {
  if (mBridge) {
    return mBridge;
  }
  ...
  nsresult rv;
  mBridge =
      do_ImportESModule("resource:///modules/BashKittenBlockerService.sys.mjs",
                        "BashKittenBlockerService", &rv);
  if (NS_FAILED(rv)) {
    mBridge = nullptr;
  }
...
NS_IMETHODIMP
BashKittenBlockerContentPolicy::ShouldLoad(nsIURI* aContentLocation,
  ...
  *aDecision = nsIContentPolicy::ACCEPT;
  ...
  nsIBashKittenBlockerContentPolicyBridge* bridge = GetBridge();
  if (!bridge) {
    return NS_OK;
  }
...
NS_IMETHODIMP
BashKittenBlockerContentPolicy::ShouldProcess(nsIURI* /* aContentLocation */,
                                            nsILoadInfo* /* aLoadInfo */,
                                            int16_t* aDecision) {
  NS_ENSURE_ARG_POINTER(aDecision);
...
BashKittenBlockerXPCOM::BashKittenBlockerXPCOM() = default;

BashKittenBlockerXPCOM::~BashKittenBlockerXPCOM() { ResetEngine(nullptr); }

void BashKittenBlockerXPCOM::ResetEngine(ContentClassifierFFIEngine* aEngine) {
  if (mEngine) {
...
NS_IMETHODIMP
BashKittenBlockerXPCOM::InitFromLists(const nsTArray<nsCString>& aFilterLists) {
...
NS_IMETHODIMP
BashKittenBlockerXPCOM::InitFromCache(const nsTArray<uint8_t>& aCacheData) {
  NS_ENSURE_TRUE(EnsureDomainResolver(), NS_ERROR_NOT_AVAILABLE);
...
NS_IMETHODIMP
BashKittenBlockerXPCOM::CheckRequestDetailed(const nsACString& aUrl,
...
NS_IMETHODIMP
BashKittenBlockerXPCOM::GetCspDirectives(const nsACString& aUrl,
                                       const nsACString& aSourceHostname,
                                       const nsACString& aHostname,
                                       const nsACString& aRequestType,
                                       bool aIsThirdParty,
...
NS_IMETHODIMP
BashKittenBlockerXPCOM::GetReplaceDirectives(const nsACString& aUrl,
                                           const nsACString& aSourceHostname,
                                           const nsACString& aHostname,
                                           const nsACString& aRequestType,
                                           bool aIsThirdParty,
...
NS_IMETHODIMP
BashKittenBlockerXPCOM::Serialize(nsTArray<uint8_t>& _retval) {
  _retval.Clear();
...
NS_IMETHODIMP
BashKittenBlockerXPCOM::EnableTags(const nsTArray<nsCString>& aTags) {
  NS_ENSURE_TRUE(mEngine, NS_ERROR_NOT_INITIALIZED);
  return content_classifier_engine_enable_tags(mEngine, &aTags);
}

NS_IMETHODIMP
BashKittenBlockerXPCOM::DisableTags(const nsTArray<nsCString>& aTags) {
  NS_ENSURE_TRUE(mEngine, NS_ERROR_NOT_INITIALIZED);
  return content_classifier_engine_disable_tags(mEngine, &aTags);
}

NS_IMETHODIMP
BashKittenBlockerXPCOM::TagExists(const nsACString& aTag, bool* _retval) {
...
NS_IMETHODIMP
BashKittenBlockerXPCOM::GetCosmeticResources(const nsACString& aUrl,
                                           nsACString& _retval) {
  _retval.Truncate();
...
NS_IMETHODIMP
BashKittenBlockerXPCOM::GetHiddenClassIdSelectors(
...
NS_IMETHODIMP
BashKittenBlockerXPCOM::UseResources(const nsACString& aResourcesJson) {
  NS_ENSURE_TRUE(mEngine, NS_ERROR_NOT_INITIALIZED);
...
extern "C" nsresult bashkitten_blocker_xpcom_constructor(REFNSIID aIID,
  ...
  *aResult = nullptr;
  ...
  RefPtr<BashKittenBlockerXPCOM> blocker = new BashKittenBlockerXPCOM();

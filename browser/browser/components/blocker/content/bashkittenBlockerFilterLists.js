...
const CATEGORY_L10N_IDS = Object.freeze({
  annoyances: "bashkitten-blocker-filter-lists-category-annoyances",
  core: "bashkitten-blocker-filter-lists-category-core",
  optional: "bashkitten-blocker-filter-lists-category-optional",
  privacy: "bashkitten-blocker-filter-lists-category-privacy",
  regional: "bashkitten-blocker-filter-lists-category-regional",
});
...
/**
 * Loads list metadata from `BashKittenBlockerService`, renders category sections
 * with toggles for each list, and saves state overrides when the dialog is
 * accepted.
 */
var gBashKittenBlockerFilterListsManager = {
  ...
  onLoad() {
    this._initialise().catch(err => {
      console.error(
        "[BashKittenBlocker] Failed to initialise filter list dialog:",
        err
      );
    });
  },
  ...
  async _loadEntries() {
    let catalog = [];
    try {
      catalog = await BashKittenBlockerService.getFilterListCatalog();
    } catch (err) {
      console.error("[BashKittenBlocker] Failed to load filter lists:", err);
    }
  ...
  _buildSections() {
    ...
    if (!this._entries.length) {
      ...
      setLabelL10nAttributes(
        emptyLabel,
        "bashkitten-blocker-filter-lists-empty-state"
      );
  ...
  _buildCategorySection(category, entries) {
    const container = createXULElement("vbox", {
      class: "bashkitten-blocker-category",
    });

    const header = createXULElement("hbox", {
      align: "center",
      "aria-expanded": "false",
      class: "bashkitten-blocker-category-header",
      role: "button",
      tabindex: "0",
    });
    ...
    const categoryLabel = createXULElement("label", {
      class: "bashkitten-blocker-category-title",
    });
    ...
    const counterLabel = createXULElement("label", {
      class: "text-deemphasized bashkitten-blocker-category-counter",
      value: "0/0",
    });
    ...
    const listContainer = createXULElement("vbox", {
      class: "bashkitten-blocker-category-lists",
    });
    ...
    for (const entry of entries) {
      const row = createXULElement("hbox", {
        align: "center",
        class: "bashkitten-blocker-list-row",
      });
      ...
      if (entry.sourceUrl) {
        const linkIcon = createXULElement("image", {
          class: "bashkitten-blocker-list-link-icon",
      ...
      const lastUpdatedLabel = createXULElement("label", {
        class: "text-deemphasized bashkitten-blocker-list-updated",
      });
  ...
  async _loadMetadata() {
    try {
      const metaList = await BashKittenBlockerService.getFilterListMetadata();
      return new Map(metaList.map(m => [m.url, m]));
    } catch (err) {
      console.error("[BashKittenBlocker] Failed to load metadata:", err);
      return new Map();
    }
  },
  ...
  _initSearch() {
    const input = document.getElementById(
      "bashkittenBlockerFilterListsSearch"
    );
  ...
  async _onRefreshNow() {
    const button = document.getElementById("bashkittenBlockerRefreshNow");
    button.disabled = true;

    try {
      await BashKittenBlockerService.refreshListsAndEngine();
      this._metadata = await this._loadMetadata();
      this._updateAllLastUpdatedLabels();
      this._updateNextRefreshLabel();
    } catch (err) {
      console.error("[BashKittenBlocker] Manual refresh failed:", err);
    } finally {
  ...
  _updateNextRefreshLabel() {
    const label = document.getElementById("bashkittenBlockerNextRefresh");
    if (!label) {
      return;
    }
...
document.addEventListener("DOMContentLoaded", () => {
  gBashKittenBlockerFilterListsManager.onLoad();
});

document.addEventListener("dialogaccept", event => {
  if (!gBashKittenBlockerFilterListsManager.onDialogAccept()) {
    event.preventDefault();
  }
});

document.addEventListener("dialogcancel", event => {
  if (!gBashKittenBlockerFilterListsManager.onDialogCancel()) {
    event.preventDefault();
  }
});

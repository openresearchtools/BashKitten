...
const CUSTOM_FILTERS_DEFAULT_FILE_NAME = "bashkitten-custom-filters.txt";
...
var gBashKittenBlockerCustomFilters = {
  _acceptButton: null,
  _dialog: null,
  _exportButton: null,
  _importButton: null,
  ...
  async init() {
    ...
    this._textarea = document.getElementById(
      "bashkittenBlockerCustomFiltersText"
    );
    this._status = document.getElementById(
      "bashkittenBlockerCustomFiltersStatus"
    );
    this._importButton = document.getElementById(
      "bashkittenBlockerCustomFiltersImport"
    );
    this._exportButton = document.getElementById(
      "bashkittenBlockerCustomFiltersExport"
    );
    ...
    this._importButton?.addEventListener("command", () => {
      this._importFromFile().catch(err => {
        console.error(
          "[BashKittenBlocker] Failed to import custom filters:",
          err
        );
        showAlert(
          "bashkitten-blocker-custom-filters-import-error-title",
          "Import failed",
          "bashkitten-blocker-custom-filters-import-error",
    ...
    this._exportButton?.addEventListener("command", () => {
      this._exportToFile().catch(err => {
        console.error(
          "[BashKittenBlocker] Failed to export custom filters:",
          err
        );
        showAlert(
          "bashkitten-blocker-custom-filters-export-error-title",
          "Export failed",
          "bashkitten-blocker-custom-filters-export-error",
    ...
    document.addEventListener("dialogaccept", event => {
      ...
      this._saveAndClose().catch(err => {
        console.error(
          "[BashKittenBlocker] Failed to save custom filters:",
          err
        );
        showAlert(
          "bashkitten-blocker-custom-filters-save-error-title",
          "Save failed",
          "bashkitten-blocker-custom-filters-save-error",
  ...
  async _load() {
    ...
    try {
      this._initialText =
        await BashKittenBlockerService.getCustomFiltersText();
      ...
      console.error("[BashKittenBlocker] Failed to load custom filters:", err);
      await showAlert(
        "bashkitten-blocker-custom-filters-load-error-title",
        "Load failed",
        "bashkitten-blocker-custom-filters-load-error",
  ...
  _updateStatus() {
    ...
    this._updateStatusLocalized(activeCount, dirty, generation).catch(err => {
      ...
      console.error(
        "[BashKittenBlocker] Failed to localize custom filters status:",
        err
      );
  ...
  async _updateStatusLocalized(activeCount, dirty, generation) {
    ...
    if (dirty) {
      const unsavedStatus = await formatValue(
        "bashkitten-blocker-custom-filters-status-unsaved",
        "Unsaved changes."
      );
  ...
  async _saveAndClose() {
    ...
    try {
      ...
      await BashKittenBlockerService.setCustomFiltersText(normalized);
  ...
  async _importFromFile() {
    const title = await formatValue(
      "bashkitten-blocker-custom-filters-import-picker-title",
    ...
    if (this._textarea.value !== this._initialText) {
      const confirmTitle = await formatValue(
        "bashkitten-blocker-custom-filters-import-replace-title",
        "Replace current custom filters?"
      );
      const confirmMessage = await formatValue(
        "bashkitten-blocker-custom-filters-import-replace-message",
  ...
  async _exportToFile() {
    const title = await formatValue(
      "bashkitten-blocker-custom-filters-export-picker-title",
...
document.addEventListener("DOMContentLoaded", () => {
  gBashKittenBlockerCustomFilters.init();
});

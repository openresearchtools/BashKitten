...
BashKitten ships the Waterfox-derived fallback blocker assets so the blocker
...
| `easylist.txt` and `easyprivacy.txt` | EasyList authors | GPL-3.0-or-later or CC BY-SA-3.0-or-later. BashKitten uses the GPL-3.0 option in `about:license`. |
...
6. Build and verify `bashkitten-blocker-assets-source.tar.xz` with
   `bashkitten/scripts/blocker_asset_provenance.py`. Release validation rejects

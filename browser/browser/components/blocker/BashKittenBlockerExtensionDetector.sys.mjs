...
ChromeUtils.defineLazyGetter(lazy, "blockerLocalization", () => {
  return new Localization(["browser/bashkitten/blocker.ftl"]);
});

const PREF_BLOCKER_ENABLED = "bashkitten.blocker.enabled";
const PREF_BLOCKER_UI_ENABLED = "bashkitten.blocker.ui.enabled";
...
const PREF_COEXIST = "bashkitten.blocker.coexist";

const DETECTION_DELAY_MS = 500;
const CHROME_DOCUMENT_LOADED_TOPIC = "chrome-document-loaded";
const PREF_CHANGED_TOPIC = "nsPref:changed";
const EXTENSION_NAME_PLACEHOLDER = "__BASHKITTEN_BLOCKER_EXTENSION_NAME__";
const L10N_ID_EXTENSION_FALLBACK_NAME_THIS =
  "bashkitten-blocker-extension-fallback-name-this";
const L10N_ID_EXTENSION_FALLBACK_NAME_YOUR =
  "bashkitten-blocker-extension-fallback-name-your";
const L10N_ID_INSTALL_WARNING = "bashkitten-blocker-extension-install-warning";
const L10N_ID_INSTALL_WARNING_MANAGE_SETTINGS =
  "bashkitten-blocker-extension-install-manage-settings";
const L10N_ID_PROMPT_TITLE = "bashkitten-blocker-prompt-title";
const L10N_ID_INSTALL_ANYWAY = "bashkitten-blocker-extension-install-anyway";
 ...
 * warnings read from.
 */
export const BashKittenBlockerExtensionDetector = {
  _addonListener: {
    onEnabled(addon) {
      BashKittenBlockerExtensionDetector._onAddonStateChanged(addon);
    },
    onDisabled(addon) {
      BashKittenBlockerExtensionDetector._onAddonStateChanged(addon);
    },
  ...
  _installListener: {
    onInstallStarted(install) {
      return BashKittenBlockerExtensionDetector._onInstallStarted(install);
    },
    onInstallEnded(install) {
      BashKittenBlockerExtensionDetector._onInstallEnded(install);
    },
  },
  ...
  async _preloadLocalizedStrings() {
    ...
    const loadPromise = (async () => {
      ...
      this._localizedStringCache.set(
        "bashkitten-blocker-extension-install-warning-template",
  ...
  init() {
    ...
    this._preloadLocalizedStrings().catch(error => {
      console.error(
        "[BashKittenBlockerExtensionDetector] Failed to preload localized strings",
        error
      );
    });
    ...
    this._refreshDetectionNotifications().catch(error => {
      console.error(
        "[BashKittenBlockerExtensionDetector] Startup detection failed",
        error
      );
    });
  },
  ...
  uninit() {
    ...
    try {
      ...
      console.warn(
        "[BashKittenBlockerExtensionDetector] Failed to remove enabled observer:",
        err
      );
    }
    ...
    try {
      ...
      console.warn(
        "[BashKittenBlockerExtensionDetector] Failed to remove dismissed observer:",
        err
      );
    }
    ...
    try {
      ...
      console.warn(
        "[BashKittenBlockerExtensionDetector] Failed to remove coexist observer:",
        err
      );
    }
  },
  ...
  _onAddonStateChanged(addon) {
    ...
    this._refreshDetectionNotifications().catch(error => {
      console.error(
        "[BashKittenBlockerExtensionDetector] Failed to refresh detection notifications",
        error
      );
    });
  },
  ...
  _onBuiltInBlockerPrefChanged() {
    ...
    this._handleBuiltInBlockerReenabled()
      ...
      .catch(error => {
        console.error(
          "[BashKittenBlockerExtensionDetector] Re-enable handling failed",
          error
        );
      });
  },
  ...
  _onDetectionDismissedPrefChanged() {
    ...
    this._refreshDetectionNotifications().catch(error => {
      console.error(
        "[BashKittenBlockerExtensionDetector] Failed to re-enable detection notifications",
        error
      );
    });
  },
  ...
  _onCoexistPrefChanged() {
    ...
    this._refreshDetectionNotifications().catch(error => {
      console.error(
        "[BashKittenBlockerExtensionDetector] Failed to refresh after coexist toggle",
        error
      );
    });
  },
  ...
  _removeStartupObserver() {
    ...
    try {
      ...
      console.warn(
        "[BashKittenBlockerExtensionDetector] Failed to remove startup observer:",
        err
      );
    }
    ...
    const progressListener = {
      onLocationChange: (browser, webProgress, _request, location) => {
        ...
        this._maybeShowDetectionUpgradeForBrowser(win, browser, location).catch(
          error => {
            console.error(
              "[BashKittenBlockerExtensionDetector] Failed to show upgrade modal",
              error
            );
          }
        );
  ...
  _seedDetectionForWindow(win, source = "seed") {
    ...
    const tryBrowser = browser => {
      this._maybeShowDetectionUpgradeForBrowser(
        ...
        console.error(
          `[BashKittenBlockerExtensionDetector] Failed to show ${source} upgrade modal`,
          error
        );
      });
    };
  ...
  async _prewarmUpgradeMessage() {
    ...
    this._upgradeMessagePromise =
      lazy.OnboardingMessageProvider.getUpgradeMessage()
        ...
        .catch(error => {
          console.error(
            "[BashKittenBlockerExtensionDetector] Failed to prewarm upgrade message",
            error
          );
          return null;
        })
  ...
  async _showDetectionUpgradeModal(win, browser, extensionName) {
    ...
    message.id = `BASHKITTEN_BLOCKER_UPGRADE_${idSuffix}`;
    ...
    screen.id = `BASHKITTEN_BLOCKER_UPGRADE_SCREEN_${idSuffix}`;

    const [spotlightTitle, spotlightPrimaryLabel, spotlightSecondaryLabel] =
      await lazy.blockerLocalization.formatValues([
        "bashkitten-blocker-spotlight-title",
        "bashkitten-blocker-spotlight-primary-button",
        "bashkitten-blocker-spotlight-secondary-button",
      ]);
    const detectedName =
      extensionName ||
    ...
    const spotlightSubtitle = await lazy.blockerLocalization.formatValue(
      "bashkitten-blocker-spotlight-subtitle",
  ...
  _onInstallStarted(install) {
    ...
    this._preloadLocalizedStrings().catch(error => {
      console.error(
        "[BashKittenBlockerExtensionDetector] Failed to refresh install warning strings",
        error
      );
    });
  ...
  async _disableAdblockExtensions(addons) {
    ...
    for (const addon of addons) {
      try {
        ...
        failed = true;
        console.error(
          `[BashKittenBlockerExtensionDetector] Failed to disable ${addon?.id || "<unknown>"}`,
          error
        );
      }
    }
  ...
  async _handleBuiltInBlockerReenabled() {
    ...
    const [promptTitle, messageText, useBuiltInLabel, keepExtensionLabel] =
      await lazy.blockerLocalization.formatValues([
        "bashkitten-blocker-prompt-title",
        {
          id: "bashkitten-blocker-reenable-conflict-message",
          args: { extensionName: conflictingName },
        },
        "bashkitten-blocker-reenable-use-built-in",
        "bashkitten-blocker-reenable-keep-extension",
      ]);
  ...
  _showInstallWarning(win, addon) {
    try {
      ...
      console.error(
        "[BashKittenBlockerExtensionDetector] Failed to show install warning prompt",
        error
      );
      return true;
    }

...
const CONTRACT_ID = "@bashkitten.invalid/bashkitten-blocker-engine;1";
...
const PREF_BRANCH = "bashkitten.blocker.";

const BLOCKED_COUNT_MAP_MAX_ENTRIES = 500;
const BLOCKED_COUNT_MAP_TRIM_TO_ENTRIES = 250;
const TOPIC_BLOCKED_COUNT_UPDATED = "BashKittenBlocker:BlockedCountUpdated";
const TOPIC_BLOCKED_COUNTS_CLEARED = "BashKittenBlocker:BlockedCountsCleared";
...
function applyReplaceDirectives(text, directives) {
  ...
  for (const directive of directives) {
    try {
      ...
      console.warn(
        "[BashKittenBlocker] Skipping invalid $replace directive:",
        err
      );
    }
  }
...
function applyHtmlFilters(text, filters, documentUrl, mimeType) {
  ...
  for (const rawFilter of filters) {
    ...
    try {
      ...
      console.warn("[BashKittenBlocker] Skipping invalid HTML filter:", err);
      continue;
    }
  ...
  onStopRequest(request, statusCode) {
    if (!this.passThrough) {
      ...
      try {
        ...
        console.warn(
          "[BashKittenBlocker] Failed to filter response body:",
          err
        );
        this.emitBytes(request, originalBytes);
      }
 ...
 */
export const BashKittenBlockerService = {
  ...
  QueryInterface: ChromeUtils.generateQI([
    "nsIObserver",
    "nsIBashKittenBlockerContentPolicyBridge",
  ]),
  ...
  _createEngine() {
    return Cc[CONTRACT_ID].createInstance(Ci.nsIBashKittenBlockerEngine);
  },
    ...
    return lazy.ListStore.withListWriteLock(async () => {
      ...
      for (const descriptor of descriptors) {
        ...
        try {
          ...
          console.warn(
            `[BashKittenBlocker] Failed to fetch list: ${descriptor.url}`,
            err
          );
        }
      }
  ...
  async _preprocessListRecords(listRecords) {
    const records =
      await lazy.ListStore.withBashKittenUnbreakRecord(listRecords);
  ...
  async _initFromTextSourcesAndCache(descriptors, generation) {
    ...
    if (storedListsUsable) {
      try {
        ...
        console.warn(
          "[BashKittenBlocker] Stored lists failed to load, trying network fetch:",
          err
        );
      }
    }
    ...
    if (fetchedListsUsable) {
      try {
        ...
        console.warn(
          "[BashKittenBlocker] Fetched lists failed to load, falling back to bundled:",
          err
        );
      }
    }
  ...
  async _doInitializeEngineIfNeeded() {
    ...
    try {
      ...
      if (e.result !== Cr.NS_ERROR_FILE_NOT_FOUND) {
        console.error("[BashKittenBlocker] Unexpected cache error:", e);
      }
    }
  ...
  _notifyBlockedCountsCleared() {
    try {
      ...
      console.warn(
        "[BashKittenBlocker] Failed to notify cleared counts:",
        err
      );
    }
  },
  ...
  _notifyBlockedCountUpdated(browserId, blockedCount) {
    try {
      ...
      console.warn("[BashKittenBlocker] Failed to notify blocked count:", err);
    }
  },
  ...
  _handleTopLevelDocumentRequest(
    ...
    try {
      ...
      console.error(
        "[BashKittenBlocker] Failed to redirect to blocked page:",
        err
      );
    ...
    try {
      ...
      console.warn(
        "[BashKittenBlocker] Failed to increment blocked count:",
        err
      );
    }
  },
  ...
  _redirectChannel(channel, target) {
    try {
      ...
      console.warn("[BashKittenBlocker] Failed to redirect channel:", err);
      return false;
    }
  },
  ...
  _onExamineResponse(subject) {
    ...
    try {
      ...
      console.error(
        "[BashKittenBlocker] Failed to apply CSP directives:",
        err
      );
    }
  },
  ...
  _maybeFilterResponseBody(
    ...
    try {
      ...
      console.warn(
        "[BashKittenBlocker] Failed to install response filter:",
        err
      );
    }
  },
  ...
  _startListUpdateTriggers() {
    ...
    this._startupCatchupTimerId = lazy.setTimeout(() => {
      ...
      this._updateListsIfNeeded().catch(err => {
        console.warn("[BashKittenBlocker] Startup list update failed:", err);
      });
    }, STARTUP_LIST_CATCHUP_DELAY_MS);

    this._fallbackUpdateIntervalId = lazy.setInterval(() => {
      this._updateListsIfNeeded().catch(err => {
        console.warn("[BashKittenBlocker] Fallback list update failed:", err);
      });
  ...
  checkRequest(url, sourceHostname, hostname, requestType, isThirdParty) {
    ...
    try {
      ...
      console.error("[BashKittenBlocker] checkRequest failed:", err);
      return this._normalizeCheckResult(null);
    }
  },
  ...
  getCosmeticResources(url, browsingContext = null) {
    ...
    try {
      ...
      console.error("[BashKittenBlocker] getCosmeticResources failed:", err);
      return null;
    }
  ...
  getCspDirectives(url, sourceHostname, hostname, requestType, isThirdParty) {
    ...
    try {
      ...
      console.error("[BashKittenBlocker] getCspDirectives failed:", err);
      return "";
    }
  },
  ...
  getReplaceDirectives(
    ...
    try {
      ...
      console.error("[BashKittenBlocker] getReplaceDirectives failed:", err);
      return [];
    }
  },
  ...
  getHtmlFilteringResources(url) {
    ...
    try {
      ...
      console.error(
        "[BashKittenBlocker] getHtmlFilteringResources failed:",
        err
      );
      return { htmlFilters: [], responseHeaderFilters: [] };
    }
  ...
  async setCustomFiltersText(text) {
    ...
    if (hadPreviousFile) {
      try {
        ...
        console.warn(
          "[BashKittenBlocker] Failed reading previous custom filters for rollback:",
          err
        );
      }
    }
    ...
    try {
      ...
      try {
        ...
        console.error(
          "[BashKittenBlocker] Failed rolling back custom filters after rebuild failure:",
          rollbackErr
        );
      }
  ...
  getHiddenClassIdSelectors(classes = [], ids = [], exceptions = []) {
    ...
    try {
      ...
      console.error(
        "[BashKittenBlocker] getHiddenClassIdSelectors failed:",
        err
      );
      return [];
    }
  ...
  _unregisterNetworkObservers() {
    ...
    for (const topic of [
      ...
      try {
        ...
        console.warn(
          `[BashKittenBlocker] Failed to remove observer for ${topic}:`,
          err
        );
      }
    }
  ...
  async _initializeEngineWithRetry() {
    try {
      ...
      console.error("[BashKittenBlocker] Failed to initialise engine:", err);
  ...
  _migrateSiteExceptions() {
    ...
    if (raw) {
      try {
        ...
        console.warn(
          "[BashKittenBlocker] Failed to parse legacy site exceptions pref:",
          err
        );
      }
    }
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        console.warn(
          `[BashKittenBlocker] Failed to migrate site exception "${entry}":`,
          err
        );
      }
    }
  ...
  async init() {
    ...
    lazy.EngineCache.cleanupStale().catch(err => {
      console.warn("[BashKittenBlocker] Cache cleanup failed:", err);
    });
    ...
    switch (data) {
      ...
      case PREF_FILTER_LIST_URLS:
        if (this.isEnabled()) {
          this.refreshListsAndEngine().catch(err => {
            console.error(
              "[BashKittenBlocker] Failed to refresh lists after pref change:",
              err
            );
          });
        }
      ...
      case PREF_ENABLED_LISTS:
        if (this.isEnabled()) {
          ...
          this.refreshListsAndEngine().catch(err => {
            console.error(
              "[BashKittenBlocker] Failed to refresh lists after list toggle:",
              err
            );
          });
        }

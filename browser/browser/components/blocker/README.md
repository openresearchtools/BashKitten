# BashKitten blocker component

This component is derived from the Waterfox blocker commits recorded in
`bashkitten/ports.toml`. The engine is Brave's `adblock-rs` v0.12.1
...
`BashKittenBlockerService` observes `http-on-modify-request`, normalises the request context, and calls `checkRequestDetailed(...)` on `nsIBashKittenBlockerEngine`. XPCOM forwards through the C++ `ContentClassifierEngine` into the Rust FFI and `adblock-rs`.

If the request matches and there is no exception, resources that are not documents are cancelled and documents loaded at the top level are redirected to `blockedPage.xhtml`. Clicking "Load anyway" goes through the `BashKittenBlockedPage` actor, records a permission for the session in `nsIPermissionManager`, and navigates to the original URL.

Normal windows use `bashkitten-blocker`, private windows use
`bashkitten-blocker-pb`, and the private permission type is cleared when the
last private context exits. Permanent user exceptions remain normal
`bashkitten-blocker` permissions: a site permanently allowed in normal
...
Custom filter list URLs come from the Custom Filter Lists dialog and live in `bashkitten.blocker.filterListUrls`, which must use HTTPS. They are fetched into the profile list cache and refresh through the same path as catalog lists.

My Filters reads from profile text at
`ProfD/bashkitten-blocker/custom-filters.txt`, using standard uBlock Origin
...
loads only these bundled copies; it has no BashKitten or Waterfox resource
update endpoint.
...
`bashkitten/scripts/blocker_asset_provenance.py` to reproduce the outputs and
publish `bashkitten-blocker-assets-source.tar.xz` with both complete upstream
...
| EasyList and EasyPrivacy fallback lists | EasyList authors | GPL-3.0-or-later or CC BY-SA-3.0-or-later | BashKitten uses the GPL-3.0 option in `about:license` |

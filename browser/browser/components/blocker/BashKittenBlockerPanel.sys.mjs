...
const PREF_UI_ENABLED = "bashkitten.blocker.ui.enabled";
...
const WIDGET_ID = "bashkitten-blocker-toolbar-button";
const PANEL_STYLESHEET_URI =
  "chrome://browser/content/blocker/bashkittenBlockerPanel.css";

const PANEL_IDS = {
  panel: "bashkitten-blocker-panel",
  multiview: "bashkitten-blocker-multiview",
  mainView: "bashkitten-blocker-mainView",
  headerSection: "bashkitten-blocker-header-section",
  header: "bashkitten-blocker-header-label",
  settingsButton: "bashkitten-blocker-settings-button",
  siteToggle: "bashkitten-blocker-panel-site-toggle",
};

const L10N_IDS = {
  notAvailable: "bashkitten-blocker-panel-not-available",
  settingsButton: "bashkitten-blocker-panel-settings-button",
  headerHost: "protections-header",
  toggle: "bashkitten-blocker-panel-toggle",
};
...
/**
 * Owns the BashKitten blocker toolbar button and popup panel.
 *
 ...
 * and routes interactions to `BashKittenBlockerService`.
 */
export const BashKittenBlockerPanel = {
  _initialized: false,
  ...
  _handleTabOpen(win, event) {
    ...
    BashKittenBlockerService.recordNewTabSourceHost(
      newBrowserId,
  ...
  _hookBrowserWindow(win) {
    ...
    const onLocationChange = (
      ...
      if (isTopLevel && !isSameDocument) {
        ...
        if (browserId) {
          BashKittenBlockerService.resetBlockedCount(browserId);
        }
      }
  ...
  _refreshPanelForWindow(win, enabled) {
    ...
    const excepted = host
      ? BashKittenBlockerService.isSiteExcepted(host, options)
      : false;
  ...
  _setSiteExceptionForCurrentSite(win, disableForSite, sourceNode = null) {
    ...
    if (this._isPrivateWindow(win)) {
      if (disableForSite) {
        BashKittenBlockerService.allowSiteForSession(host, {
          isPrivate: true,
        });
      } else {
        BashKittenBlockerService.removeSiteException(host, {
          isPrivate: true,
        });
      }
    } else if (disableForSite) {
      BashKittenBlockerService.addSiteException(host);
    } else {
      BashKittenBlockerService.removeSiteException(host);
    }
  ...
  uninit() {
    ...
    for (const topic of OBSERVED_TOPICS) {
      try {
        ...
        console.warn(
          `[BashKittenBlockerPanel] Failed to remove observer for ${topic}:`,
          err
        );
      }
    }
    ...
    try {
      ...
      console.warn(
        "[BashKittenBlockerPanel] Failed to remove pref observer:",
        err
      );
    }

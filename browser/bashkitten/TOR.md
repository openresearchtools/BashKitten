...
BashKitten routes Tor tabs through the bundled C Tor client.
...
Shell agents can use `bashkitten onion-auth list`, `onion-auth remove ADDRESS`,
and `bashkitten --input - onion-auth set`. The set command reads a JSON object
...
`bashkitten onion-auth privacy ADDRESS --private-mode false` to change the
...
in `bashkitten/third_party/tor.toml`. Update that pin and the vendored tree from
an official stable release together; do not substitute a distribution binary.

Build with `bashkitten/scripts/build-tor-runtime.sh --build-root DIR` in the
...
are recorded in `bashkitten/third_party/tor-notices/THIRD-PARTY.json`.

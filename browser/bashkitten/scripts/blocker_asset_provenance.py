...
ARCHIVE_ROOT = "bashkitten-blocker-assets-source"
...
def repository_paths(repository):
    ...
    return {
        ...
        "notice": repository / "bashkitten" / "BLOCKER-ASSET-SOURCE-NOTICE",
...
def load_lock(path):
    ...
    if artifact != "bashkitten-blocker-assets-source.tar.xz":
...
def download_source(source, cache_dir):
    ...
    request = urllib.request.Request(
        source["archiveUrl"], headers={"User-Agent": "BashKitten release builder"}
    )
    try:
...
def create_source_bundle(repository, cache_dir, output, node, reproduce=True):
    ...
    source_roots = {}
    license_values = {}
    with tempfile.TemporaryDirectory(prefix="bashkitten-blocker-source-") as temp:
...
def verify_source_bundle(path, repository):
    ...
    for key in SOURCE_KEYS:
        ...
        with tempfile.TemporaryDirectory(prefix="bashkitten-blocker-verify-") as temp:

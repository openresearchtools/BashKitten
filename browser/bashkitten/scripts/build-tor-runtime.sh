...
build_root="$(dirname -- "${source_repo}")/bashkitten-tor-builds"
...
metadata="${source_repo}/bashkitten/third_party/tor.toml"
...
source_archive="${run_root}/artifacts/bashkitten-tor-${version}-source.tar"
...
make -j"${BASHKITTEN_BUILD_JOBS:-8}" > "${run_root}/build.log" 2>&1
...
provenance="${run_root}/artifacts/bashkitten-tor-${version}-provenance.zip"

...
EXTENSIONS_ROOT = REPOSITORY / "bashkitten" / "browser" / "extensions"
...
WEB = ExtensionConfig(
    slug="web-search",
    repository="bashkitten-extensions",
    source_path="extensions/web-search",
    extension_id="web-search@extensions.bashkitten",
...
TORRENT = ExtensionConfig(
    ...
    extension_id="torrent-search@extensions.bashkitten",
...
def torrent_manifest(version):
    return {
        ...
        "default_locale": "en",
        "incognito": "not_allowed",
        "author": "BashKitten contributors",
        "browser_specific_settings": {
            "gecko": {
...
def render_moz_build(config):
    ...
    lines = [
        "# SPDX-License-Identifier: AGPL-3.0-or-later",
        "",
        "# Generated by bashkitten/scripts/sync_builtin_search_extensions.py.",
    ]
...
def resolve_source(args, config):
    ...
    if source is None and os.environ.get("BASHKITTEN_EXTENSIONS_SOURCE"):
        source = Path(os.environ["BASHKITTEN_EXTENSIONS_SOURCE"])
    if source is None:

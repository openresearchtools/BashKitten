...
import { BashKittenBrowserStyle } from "resource:///modules/BashKittenBrowserStyle.sys.mjs";
import { BashKittenThemeColors } from "resource:///modules/BashKittenThemeColors.sys.mjs";

const NOVA_PREF = "browser.nova.enabled";
const BROWSER_STYLE_PREF = "browser.theme.bashkitten.browserStyle";
const STYLE_PREF = "browser.theme.enableBashKittenCustomizations";
// Lepton modes: 0/1 load the BashKitten chrome customisations, 2 turns them off.
const LEPTON_ON = 1;
const LEPTON_OFF = 2;
...
// The theme drives Lepton: only Photon uses the BashKitten chrome tab styling;
// Nova and Proton run on stock Firefox chrome (Lepton off).
function setStyle(style) {
  switch (style) {
    case "nova":
      BashKittenBrowserStyle.applyStockTabStyle();
    ...
    case "proton":
      ...
      break;
    case "photon":
      BashKittenBrowserStyle.applyPhotonTabStyle();
      Services.prefs.setStringPref(BROWSER_STYLE_PREF, style);
...
function setThemeMode(mode) {
  BashKittenThemeColors.setMode(mode);
}

function setThemeColor(color) {
  BashKittenThemeColors.setColor(color);
}
...
function keepPrivacyDefaults() {
  Services.prefs.setBoolPref("bashkitten.blocker.enabled", true);
}

export const BashKittenOnboardingActions = {
  async handle(data = {}) {
    switch (data.action) {
      case "style":
        setStyle(data.value);

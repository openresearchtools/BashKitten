...
async function refreshAuthorizations() {
  ...
  for (const entry of await OnionAuthStore.list()) {
    ...
    document.l10n.setAttributes(
      privacyText,
      "bashkitten-onion-auth-private-mode"
    );
    ...
    privacy.addEventListener("change", async () => {
      ...
      try {
        ...
        document.l10n.setAttributes(
          ...
          "bashkitten-onion-auth-save-error"
        );
      }
    });
    for (const action of ["edit", "remove"]) {
      ...
      button.type = "button";
      document.l10n.setAttributes(button, `bashkitten-onion-auth-${action}`);
...
window.addEventListener("DOMContentLoaded", async () => {
  document.l10n.setAttributes(
    ...
    parameters.manage
      ? "bashkitten-onion-auth-save"
      : "bashkitten-onion-auth-connect"
  );
  ...
  if (parameters.manage) {
    ...
    try {
      ...
      document.l10n.setAttributes(
        document.getElementById("feedback"),
        "bashkitten-onion-auth-unlock-error"
      );
    }
  }
  document.subDialogSetDefaultFocus();
...
async function save(event) {
  ...
  try {
    ...
    document.l10n.setAttributes(
      document.getElementById("feedback"),
      "bashkitten-onion-auth-saved-status"
    );
  } catch {
    document.l10n.setAttributes(
      document.getElementById("feedback"),

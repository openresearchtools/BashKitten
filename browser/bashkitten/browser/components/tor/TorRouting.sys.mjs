...
const CONTAINER_ID_PREF = "bashkitten.tor.containerId";
...
export const TorRouting = {
  ...
  onWindowOpened(win) {
    ...
    if (menu) {
      ...
      item.id = "bashkitten-onion-authorizations";
      win.document.l10n.setAttributes(item, "bashkitten-onion-auth-menu");
  ...
  _markTorTab(tab) {
    ...
    tab.toggleAttribute("bashkitten-tor", true);
  ...
  async _startProxy() {
    const root = PathUtils.join(PathUtils.profileDir, "bashkitten-tor");
  ...
  _isolationKey(channel) {
    ...
    if (!permanentKey) {
      return `container-${channel.loadInfo?.originAttributes.userContextId ?? this.userContextId}`;
    }
  ...
  observe(subject, topic, data) {
    if (topic == "bashkitten-onion-authorization-needed") {
    ...
    Services.obs.removeObserver(this, "bashkitten-onion-authorization-needed");

...
function wrapHomepageNewTabs(config) {
  if (config._bashkittenCustomNewTab) {
    return;
  }
  config._bashkittenCustomNewTab = true;
  ...
  config.getControlConfig = (controlConfig, deps, setting) => {
    ...
    return {
      ...
      options: [
        ...
        {
          value: "custom",
          l10nId: "bashkitten-home-mode-choice-custom-new-tab-url",
        },
        ...rest,
      ],
    };
...
const CUSTOM_NEWTAB_ITEM = {
  ...
  l10nId: "bashkitten-home-new-tab-custom-url",
  controlAttrs: {
  ...
  items: [
    {
      ...
      l10nId: "bashkitten-home-new-tab-custom-url-input",
    },
  ],
};
...
function injectCustomNewTabItem(config) {
  ...
  if (
    config.items.some(item => item.id === "bashkittenCustomNewTabUrlGroup")
  ) {
    return;
  }
...
// Firefox brand; BashKitten uses neutral wording.

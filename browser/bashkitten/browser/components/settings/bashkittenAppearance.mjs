...
const BASHKITTEN_THEME_MODE_PREF = "browser.theme.bashkitten.mode";

const THEME_MODES = [
  {
    value: "system",
    l10nId: "bashkitten-appearance-theme-mode-option-system",
    imageSrc: "chrome://browser/skin/device-desktop.svg",
  },
  {
    value: "light",
    l10nId: "bashkitten-appearance-theme-mode-option-light",
    imageSrc: "chrome://browser/skin/weather/sunny.svg",
  },
  {
    value: "dark",
    l10nId: "bashkitten-appearance-theme-mode-option-dark",
    imageSrc: "chrome://browser/skin/weather/night-hazy-moonlight.svg",
  },
];

/** Theme mode segmented control for about:preferences. */
class BashKittenModeSegmented extends HTMLElement {
  ...
  #render() {
    ...
    const labelId = `${this.id || "bashkitten-mode-segmented"}-label`;
    const label = this.ownerDocument.createElement("span");
    label.className = "bashkitten-mode-segmented-label";
    label.id = labelId;
    ...
    group.className = "bashkitten-mode-segmented-options";
    ...
    for (let option of this.#options) {
      ...
      button.type = "button";
      button.className = "bashkitten-mode-segmented-option";
      button.dataset.value = option.value;
      button.value = option.value;
      button.setAttribute("role", "radio");

      const icon = this.ownerDocument.createElement("img");
      icon.className = "bashkitten-mode-segmented-icon";
      icon.src = option.imageSrc;
      icon.alt = "";

      const text = this.ownerDocument.createElement("span");
      text.className = "bashkitten-mode-segmented-text";
  ...
  get #buttons() {
    return Array.from(
      this.querySelectorAll(".bashkitten-mode-segmented-option")
    );
  }
  ...
  #handleClick(event) {
    ...
    const button = event.target.closest(".bashkitten-mode-segmented-option");
  ...
  #handleKeyDown(event) {
    ...
    const currentButton = Element.isInstance(event.target)
      ? event.target.closest(".bashkitten-mode-segmented-option")
      : null;
...
if (!customElements.get("bashkitten-mode-segmented")) {
  customElements.define("bashkitten-mode-segmented", BashKittenModeSegmented);
}
...
Preferences.addSetting({
  id: "bashkitten-theme-mode",
  get: () => lazy.BashKittenThemeColors.getMode(),
  async set(val) {
    ...
    lazy.BashKittenThemeColors.setColor("default");
    return lazy.BashKittenThemeColors.setMode(val);
  },
  setup: emitChange =>
    observeBranches([BASHKITTEN_THEME_MODE_PREF], emitChange),
});

SettingGroupManager.registerGroups({
  bashkittenThemeColors: {
    l10nId: "bashkitten-appearance-mode-group",
    headingLevel: 2,
    items: [
      {
        id: "bashkitten-theme-mode",
        l10nId: "bashkitten-appearance-theme-mode-picker",
        control: "bashkitten-mode-segmented",
        controlAttrs: {
          class: "bashkitten-mode-segmented",
          ".options": THEME_MODES,
...
(function injectAppearanceSheet() {
  ...
  const href =
    "chrome://browser/content/bashkitten/settings/bashkittenAppearance.css";

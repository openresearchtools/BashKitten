...
Decorative upstream art is replaced by the existing BashKitten logo. No
...
def main():
    manifest_path=ROOT/'bashkitten/android/ui-artwork.json'
    ...
    for src in sorted(inputs):
        if not src.parent.name.startswith(('drawable','mipmap')) or src.name=='bashkitten_logo.png':continue
        ...
        if decorative:
            ...
            aliases.add(name)
            row['action']='BashKitten logo alias'
            ...
            row['action']='solid BashKitten background'
    ...
    for flavor in ('debug', 'beta', 'nightly', 'release'):
        ...
        for rel in originals:
            ...
            manifest[rel] = {'sha256': hashlib.sha256(data).hexdigest(), 'type': src.suffix, 'action': 'remove flavor override; inherit BashKitten branding'}
            src.unlink(missing_ok=True)
    # Launcher resources inherit the same brand in every build channel.
    (RES / 'values/bashkitten_launcher.xml').write_text(HEADER + '<resources><item type="mipmap" name="ic_launcher">@drawable/bashkitten_logo</item><item type="mipmap" name="ic_launcher_round">@drawable/bashkitten_logo</item></resources>\n')

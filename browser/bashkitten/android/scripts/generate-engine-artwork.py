...
The renderer only consumes our SVG geometry or the existing BashKitten logo.
...
spec=importlib.util.spec_from_file_location('art',ROOT/'bashkitten/android/scripts/generate-ui-artwork.py')
...
def raster(svg,width,height):
 return subprocess.check_output([os.environ.get('BASHKITTEN_RSVG_CONVERT','rsvg-convert'),'--width',str(width),'--height',str(height)],input=svg.encode())
...
def main():
 ...
 for member,(source,key) in MAPPINGS.items():
  ...
  if key=='spinner':
   with tempfile.TemporaryDirectory(prefix='bashkitten-spinner-') as temporary:
    frames=[]
   ...
   p.write_bytes((ROOT/'bashkitten/browser/branding/content/about-logo.svg').read_bytes())
  elif key=='brand-webp':
   subprocess.run(['magick',str(ROOT/'mobile/android/fenix/app/src/main/res/drawable-nodpi/bashkitten_logo.png'),'-strip','-define','webp:lossless=true',str(p)],check=True)
  else:
 ...
 path=ROOT/'bashkitten/android/ui-artwork.json';manifest=json.loads(path.read_text());manifest['engine_resources']=rows

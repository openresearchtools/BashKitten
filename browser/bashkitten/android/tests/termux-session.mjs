...
import { browserCall, sessionLocation } from '@openresearchtools/pi-bashkitten/client.mjs';
...
try {
  ...
  const loader = new DefaultResourceLoader({ cwd: directory, agentDir: join(directory, 'pi'),
    additionalExtensionPaths: [resolve('node_modules/@openresearchtools/pi-bashkitten/extension.mjs')],
    noSkills: true, noPromptTemplates: true, noThemes: true, noContextFiles: true });
  await loader.reload();
  const loaded = loader.getExtensions();
  assert.deepEqual(loaded.errors, []);
  const extension = loaded.extensions.find(item => item.tools.has('bashkitten_browser'));
  assert(extension);
  ...
  const call = async (method, params = {}, session = a) => (await tool('bashkitten_browser', { method, params }, session)).details.result;
  ...
  await check('browser-owned CLI prints its offline license notices', async () => {
    ...
    const { stdout } = await execute('pm', ['path', 'org.openresearchtools.bashkitten']);
    const env = { ...process.env, CLASSPATH: stdout.trim().replace(/^package:/, '') };
    delete env.LD_PRELOAD; delete env.LD_LIBRARY_PATH;
    const printed = await execute('/system/bin/app_process', ['/', 'org.openresearchtools.bashkitten.BrowserCommand', '--licenses'], { env, maxBuffer: 5000000 });
  ...
  await check('DOM references fill, click, wait, read, console and per-tab policies', async () => {
    ...
    await value(tabId, 'console.log("bashkitten-termux-check"); return true;');
    assert.match(JSON.stringify(await call('console', { tabId })), /bashkitten-termux-check/);
  ...
  await check('private screenshot returned as a Pi image and read again with Pi read', async () => {
    const result = await tool('bashkitten_screenshot', { tabId });
  ...
  await check('two native Pi sessions cannot list or close one another’s tabs', async () => {
    ...
    await value(tabId, 'localStorage.setItem("bashkitten-session-test", "session-a"); return true;');
    ...
    assert.equal((await call('evaluate', { tabId: second, code: 'return localStorage.getItem("bashkitten-session-test");' }, b)).value, null);
  ...
  await check('real browser download, private Pi file and bearer-protected wget', async () => {
    const contents = 'BashKitten ' + vendor + ' real browser download\n';
    ...
    const download = await poll(async () => {
      const items = (await tool('bashkitten_downloads', { action: 'list' })).details.downloads;
    ...
    const result = await tool('bashkitten_downloads', { action: 'fetch', downloadId: download.id });
    ...
    assert(!(await tool('bashkitten_downloads', { action: 'list' }, b)).details.downloads.some(item => item.id === download.id));
    await assert.rejects(tool('bashkitten_downloads', { action: 'fetch', downloadId: download.id }, b));

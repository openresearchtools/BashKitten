...
try:
    ...
    if not (root/'ca.pem').exists():
        openssl('req','-x509','-newkey','rsa:2048','-nodes','-days','30','-subj','/CN=BashKitten Test Private CA','-addext','basicConstraints=critical,CA:TRUE','-addext','keyUsage=critical,keyCertSign,cRLSign','-keyout',root/'ca.key','-out',root/'ca.pem')

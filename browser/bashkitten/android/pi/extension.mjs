...
export default function wildBuzzard(pi) {
  pi.registerTool({
    name: 'bashkitten_browser', label: 'BashKitten',
    description: 'Control the Android browser for this Pi chat. Create a tab with tabs.create {url}; show it with tabs.show {tabId}; inspect snapshot {tabId,depth,maxNodes}; if truncated, request snapshot {tabId,target} with a container reference to inspect that subtree; use act {tabId,kind,target,value} with an opaque reference from that snapshot. navigate uses {tabId,url}. evaluate uses {tabId,code} with a JavaScript function body and return. Tabs and downloaded files are isolated to this chat. Use capabilities for supported methods and diagnostics for actual remote-debugging flags. Approval, when required, is once per Android app through /bashkitten-authorize or browser Settings > Agent access.',
    promptSnippet: 'Browse real websites in Android BashKitten with DOM trees and page actions',
    promptGuidelines: ['Use bashkitten_browser snapshots to obtain element references before acting; take a fresh snapshot after navigation.',
      ...
      'Use bashkitten_screenshot for images and bashkitten_downloads to copy browser downloads into this chat’s private session storage.'],
  ...
  pi.registerTool({
    ...
    description: 'List this chat’s browser downloads, or fetch a completed download by downloadId into this Pi session. The result gives an absolute private file path usable by read. Pending downloads can be accepted with bashkitten_browser downloads.accept {tabId,downloadId}.',
  ...
  pi.registerCommand('bashkitten-authorize', {
    description: 'Allow this terminal app to use BashKitten; publisher-signed apps are allowed automatically',
    async handler(_args, ctx) {
      await browserCall(null, {}, { authorize: true });
      ctx.ui.notify('BashKitten access enabled for this Android app', 'info');
    },
  });
}

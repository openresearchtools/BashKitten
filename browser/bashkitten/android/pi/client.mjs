...
const browserPackage = 'org.openresearchtools.bashkitten';
...
export function sessionLocation(manager) {
  ...
  return { scope, directory: resolve(manager.getSessionDir(), 'bashkitten', scope) };
}
...
export async function browserCall(method, params = {}, { scope, output, signal, authorize = false } = {}) {
  ...
  if (!apk || !apk.startsWith('/data/app/')) throw new Error('Install BashKitten for Android first');
  const env = { ...process.env, CLASSPATH: apk };
  delete env.LD_PRELOAD; delete env.LD_LIBRARY_PATH;
  const args = ['/', 'org.openresearchtools.bashkitten.BrowserCommand'];

...
/** Executes content-process portions of BashKitten browser tools. */
export class BashKittenBrowserControlChild extends JSWindowActorChild {
  #consoleAPIListener;
  #consoleListener;
  #consoleMessages = [];
  #consoleMessageBytes = [];
  ...
  async receiveMessage(message) {
    if (this.isProtected()) throw new Error("Protected Agent content cannot be automated");
    switch (message.name) {
      case "snapshot":
      ...
      default:
        throw new Error(`Unknown BashKitten content command: ${message.name}`);
    }
  }

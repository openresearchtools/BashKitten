...
class NavigationTest {
    @Test fun storeAndUpdatesReturnToTheSameChat() {
        ...
        ActivityScenario.launch(MainActivity::class.java).use { scenario ->
            ...
            device.swipe(device.displayWidth / 2, device.displayHeight * 4 / 5, device.displayWidth / 2, device.displayHeight / 2, 20)
            device.findObject(By.desc("Expand packages"))?.click()
            device.waitForIdle()
            UiScrollable(UiSelector().scrollable(true)).scrollTextIntoView("Installed packages ⌄")
            val installed = device.findObject(By.text("Installed packages ⌄"))
            if (installed == null) device.takeScreenshot(File(instrumentation.targetContext.getExternalFilesDir(null), "packages-navigation-failure.png"))
            assertNotNull("The expanded Packages block must expose installed packages", installed)
            installed!!.click()
